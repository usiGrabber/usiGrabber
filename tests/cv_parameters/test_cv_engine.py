...
def test_cv_injector_creates_cv_params(engine: Engine, sample_project: Project):
    """Test that CVInjector creates new cv_params."""

    async def test_impl():
        async with CVInjector(sample_project.accession, engine) as injector:
...
def test_cv_injector_links_params_to_project(engine: Engine, sample_project: Project):
    ...
    with Session(engine) as session:
        project = session.get(Project, sample_project.accession)
        assert project is not None

        # Verify cv_param is linked to project
        assert len(project.cv_params) == 1
        assert project.cv_params[0].accession == "MS:1000463"
        assert project.cv_params[0].value == "Orbitrap"


def test_cv_injector_reuses_existing_cv_params(engine: Engine, sample_project: Project):
...
def test_cv_injector_avoids_duplicate_links(engine: Engine, sample_project: Project):
    ...
    with Session(engine) as session:
        project = session.get(Project, sample_project.accession)
        assert project is not None
        initial_count = len(project.cv_params)
    ...
    with Session(engine) as session:
        project = session.get(Project, sample_project.accession)
        assert project is not None
        assert len(project.cv_params) == initial_count
        assert len(project.cv_params) == 1


def test_cv_injector_handles_cv_tuples(engine: Engine, sample_project: Project):
    ...
    with Session(engine) as session:
        ...
        assert len(project.cv_params) == 1
        assert project.cv_params[0].accession == "MS:1000032"


def test_cv_injector_handles_none_values(engine: Engine, sample_project: Project):
    ...
    with Session(engine) as session:
        ...
        assert len(project.cv_params) == 1
        assert project.cv_params[0].accession == "MS:1000463"
        assert project.cv_params[0].value is None


def test_cv_injector_buffer_flush(engine: Engine, sample_project: Project):
    ...
    with Session(engine) as session:
        ...
        assert len(project.cv_params) == 250


def test_cv_injector_shared_cv_param_across_projects(engine: Engine):
    ...
    async def test_impl1():
        async with CVInjector("PXD000001", engine) as injector:
            await injector.add(CVParam(accession="MS:1000463", value="Orbitrap"))

    async def test_impl2():
        async with CVInjector("PXD000002", engine) as injector:
            await injector.add(CVParam(accession="MS:1000463", value="Orbitrap"))
    ...
    with Session(engine) as session:
        ...
        project1 = session.get(Project, "PXD000001")
        project2 = session.get(Project, "PXD000002")

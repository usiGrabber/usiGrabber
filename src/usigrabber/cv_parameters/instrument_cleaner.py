...
def clean_instruments_sync(instruments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """
    Synchronous version of clean_instruments that only handles Case 1 (duplicates).

    This version does not attempt to resolve instrument names via ontology lookup,
    making it suitable for synchronous code paths where ontology loading is not possible.

    Args:
        instruments: List of instrument CV parameter dicts from PRIDE API

    Returns:
        Cleaned list of instrument dicts (with duplicates removed)
    """
    if not instruments:
        return instruments
    ...
    for generic in generic_entries:
        ...
        # Cannot resolve without async, keep original
        cleaned.append(generic)

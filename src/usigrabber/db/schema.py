...
class CvParam(SQLModel, table=True):
    ...
    projects: list["Project"] = Relationship(
        back_populates="cv_params", link_model=CvJunctionTable
    )
...
class Project(SQLModel, table=True):
    ...
    cv_params: list[CvParam] = Relationship(
        back_populates="projects", link_model=CvJunctionTable
    )
...
class Peptide(SQLModel, table=True):
    ...
    peptide_modifications: list["PeptideModification"] = Relationship(
        back_populates="peptide"
    )
...
class PeptideModification(SQLModel, table=True):
    ...
    unimod_id: int = Field(
        description="Unimod id, e.g., '35' for 'UNIMOD:35' accession"
    )
    position: int = Field(description="Position in the peptide sequence (1-indexed)")
    modified_residue: str = Field(
        description="The specific amino acid that was modified"
    )
...
class PeptideSpectrumMatch(SQLModel, table=True):
    ...
    rank: int | None = Field(
        default=None, description="Rank of this PSM for the spectrum"
    )
...
class PeptideEvidence(SQLModel, table=True):
    ...
    protein_accession: str | None = Field(
        default=None, description="Protein accession."
    )
    ...
    end_position: int | None = Field(
        default=None, description="End position in protein sequence"
    )
...
class PSMPeptideEvidence(SQLModel, table=True):
    ...
    peptide_evidence: "PeptideEvidence" = Relationship(

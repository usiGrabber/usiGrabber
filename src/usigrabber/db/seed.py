...
def seed_minimal_data(engine: Engine) -> None:
    ...
    with Session(engine) as session:
        ...
        psm_evidence1 = PSMPeptideEvidence(
            psm_id=psm1.id, peptide_evidence_id=evidence1.id
        )
        psm_evidence2 = PSMPeptideEvidence(
            psm_id=psm2.id, peptide_evidence_id=evidence2.id
        )
        psm_evidence3 = PSMPeptideEvidence(
            psm_id=psm3.id, peptide_evidence_id=evidence3.id
        )

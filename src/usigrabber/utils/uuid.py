"""
This module is a copy of the standard library 'uuid' module from Python 3.14.
It provides support for UUIDv7, which we need in our project. Once Python 3.14
is the minimum required version for this project, this file should be removed and
the standard library 'uuid' module should be used instead.
"""
...
from uuid import UUID as _stdlib_UUID
...
class UUID:
    ...
    def __setstate__(self, state):
        ...
        object.__setattr__(
            self, "is_safe", SafeUUID(state["is_safe"]) if "is_safe" in state else SafeUUID.unknown
        )
...
def uuid5(namespace, name) -> _stdlib_UUID:
...
def main():
    ...
    parser.add_argument(
        "-N", "--name", help="uuid3/uuid5 only: name used as part of generating the UUID"
    )
    parser.add_argument(
        "-C", "--count", metavar="NUM", type=int, default=1, help="generate NUM fresh UUIDs"
    )
